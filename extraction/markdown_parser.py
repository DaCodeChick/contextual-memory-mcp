...
EXPLICIT_RE = re.compile(r"(?im)^\s*(?:concepts?|tags?|keywords?)\s*:\s*(.+)$")
WORD_RE = re.compile(r"[A-Za-z][A-Za-z0-9_-]{3,}")
STOPWORDS = {
    "about", "after", "again", "also", "and", "are", "because", "before", "being",
    "between", "both", "but", "can", "could", "does", "each", "from", "have", "into",
    "keep", "make", "must", "not", "only", "other", "same", "should", "that", "the",
    "their", "them", "then", "there", "these", "they", "this", "through", "use", "using",
...
def load_document(path: Path, root: Path) -> SourceDocument:
    ...
    title = heading.group(2).strip() if heading else path.stem.replace("_", " ").replace("-", " ")
...
def extract_concepts(text: str, heading: str | None = None, limit: int = 18) -> list[str]:
...
def _sections(text: str) -> list[tuple[str | None, int, int, str]]:
    ...
    if matches[0].start() > 0 and text[:matches[0].start()].strip():
        sections.append((None, 0, matches[0].start(), text[:matches[0].start()]))
    for index, match in enumerate(matches):
        start = match.end()
        end = matches[index + 1].start() if index + 1 < len(matches) else len(text)
        sections.append((match.group(2).strip(), start, end, text[start:end]))
    return sections


def _window(text: str, base: int, size: int, overlap: int):
    cursor = 0
...
def segment_document(doc: SourceDocument, size: int, overlap: int) -> list[PromptSegment]:
    result: list[PromptSegment] = []
    ordinal = 0
    for heading, start, _end, body in _sections(doc.content):
        for char_start, char_end, text in _window(body, start, size, overlap):
            concepts = extract_concepts(text, heading)
            importance = 1.4 if heading and any(k in heading.lower() for k in ("rule", "required", "constraint", "identity")) else 1.0
            result.append(PromptSegment(
                segment_id=stable_id("seg", doc.source_id, str(ordinal), content_hash(text)),
